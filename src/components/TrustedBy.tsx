...
export default function TrustedBy() {
  ...
  return <section className="overflow-hidden px-[0px] py-0">
      <div className="max-w-7xl mx-auto px-0 sm:px-6 lg:px-0">
        ...
        scrollBehavior: "auto"
          ...
          {[...partners, ...partners].map((partner, index) => <div key={`${partner.name}-${index}`} className="flex-shrink-0 hover:scale-110 transition-all duration-300 cursor-pointer">
              <img src={partner.logo} alt={`${partner.name} logo`} loading="lazy" decoding="async" className="h-20 md:h-24 lg:h-28 w-auto object-contain" />
            </div>)}
        </div>
      </div>
    </section>;
