...
const navItems = [
  ...
  { icon: Phone, label: 'Call Us', path: 'tel:+97143296840', isExternal: true },
  { icon: Users, label: 'About', path: '/company' },
  { icon: MapPin, label: 'Location', path: '/contact' },
  { icon: MessageCircle, label: 'Chat', path: 'https://wa.me/971567215617', isExternal: true },
];
...
const FloatingNavBar = () => {
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50">
      {/* Glass container */}
      <div className="relative flex items-center gap-2 px-6 py-4 rounded-full bg-background/40 backdrop-blur-xl border border-white/10 shadow-2xl">
        {/* Background glow */}
        ...
        {navItems.map((item, index) => {
          ...
          const buttonContent = (
            ...
            >
              {/* Tooltip */}
              <div 
                className={`absolute -top-10 left-1/2 -translate-x-1/2 px-3 py-1.5 rounded-lg bg-background/90 backdrop-blur-sm border border-white/10 text-xs font-medium text-foreground whitespace-nowrap transition-all duration-300 ${
              ...
              >
                ...
                <div 
                  className={`absolute inset-0 rounded-full transition-all duration-300 ${
                    isActive || isHovered
                      ? 'bg-primary/30 blur-lg opacity-100 scale-110'
                      : 'opacity-0 scale-100'
                  }`}
                />
