...
export default function HeroSection() {
  ...
  return <section className="relative min-h-screen overflow-hidden py-0">
      {/* Background Image */}
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-32 lg:pt-40">
        <div className="grid lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-3 space-y-2">
            {heroTabs.map((tab, index) => <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`hero-tab w-full text-left ${activeTab === tab.id ? 'active' : ''}`} style={{
            animationDelay: `${index * 100}ms`
          }}>
                {tab.label}
              </button>)}
            <Link to="/use-cases" className="hero-tab w-full flex items-center gap-2 text-primary hover:text-primary/80 mt-4">
              View All Use Cases
