...
const FAQSection = () => {
  return <section className="relative py-24 sm:py-32 overflow-hidden rounded-3xl shadow-inner border-double border-0">
        ...
        <div className="grid md:grid-cols-2 gap-6 lg:gap-8">
          ...
          <div className="space-y-6">
            <Accordion type="single" collapsible className="space-y-6">
              {faqs.slice(0, 3).map((faq, index) => <motion.div key={index} initial={{
              opacity: 0,
              x: -20
            }} whileInView={{
              opacity: 1,
              ...
              delay: index * 0.1
                  ...
                  <AccordionItem value={`item-${index}`} className={`bg-white/60 backdrop-blur-md rounded-3xl border border-white/80 shadow-[0_4px_24px_-4px_rgba(0,0,0,0.03)] overflow-hidden group transition-all duration-500 hover:scale-[1.02] hover:-translate-y-1 ${faq.glowColor}`}>
          ...
          <div className="space-y-6">
            <Accordion type="single" collapsible className="space-y-6">
              {faqs.slice(3, 6).map((faq, index) => <motion.div key={index + 3} initial={{
              opacity: 0,
              x: 20
            }} whileInView={{
              opacity: 1,
              ...
              delay: (index + 3) * 0.1
                  ...
                  <AccordionItem value={`item-${index + 3}`} className={`bg-white/60 backdrop-blur-md rounded-3xl border border-white/80 shadow-[0_4px_24px_-4px_rgba(0,0,0,0.03)] overflow-hidden group transition-all duration-500 hover:scale-[1.02] hover:-translate-y-1 ${faq.glowColor}`}>
                    <AccordionTrigger className="px-8 py-6 text-slate-900 hover:no-underline [&[data-state=open]>div>div:first-child]:scale-110 [&[data-state=open]>div>div:first-child]:bg-white [&[data-state=open]>svg]:rotate-180">
                      <div className="flex items-center gap-5 w-full">
                        <div className={`w-12 h-12 rounded-2xl ${faq.bgColor} flex items-center justify-center transition-all duration-500 shadow-inner group-hover:scale-110`}>
                          <faq.icon className={`w-6 h-6 ${faq.color}`} />
