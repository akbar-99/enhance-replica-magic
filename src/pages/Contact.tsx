...
export default function Contact() {
  ...
  return <main className="min-h-screen page-bg-light">
      <SEO title="Contact Us | ENHANCE TECH Dubai" description="Get in touch with ENHANCE TECH. Contact us for IT solutions, cybersecurity services, and enterprise technology support in Dubai, UAE. Phone: +97143296840" keywords="contact ENHANCE TECH, IT support Dubai, cybersecurity contact, technology consultation UAE" canonicalUrl="https://itenhance.tech/contact" structuredData={contactSchema} />
      <Navbar />
      ...
      <section className="pt-32 pb-20 page-section-blue relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          <h1 className="section-title bg-clip-text text-transparent bg-gradient-to-r from-cyan-300 to-blue-500 mb-6 animate-fade-up" style={{
          animationDelay: "0.1s"
        }}>
            Contact Us
          </h1>
      ...
      <section className="py-16 page-section-white border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-3 gap-6">
            {contactOptions.map((option, index) => <div key={option.title} className="group bg-white border border-slate-200/80 rounded-3xl p-8 text-center hover:border-transparent hover:shadow-xl hover:shadow-primary/10 transition-all duration-500 hover:-translate-y-1">
                ...
                {option.type === "phone" ? <a href={`tel:${option.contact}`} className="inline-flex items-center gap-2 text-primary hover:underline font-semibold">
                    <Phone className="w-4 h-4" />
                    {option.contact}
                  </a> : option.type === "email" ? <a href={`mailto:${option.contact}`} className="inline-flex items-center gap-2 text-primary hover:underline font-semibold">
                    <Mail className="w-4 h-4" />
                    {option.contact}
                  </a> : <span className="inline-flex items-center gap-2 text-primary font-semibold text-sm">
          ...
          <div className="grid lg:grid-cols-2 gap-8 items-start">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid sm:grid-cols-2 gap-6">
                      <div>
                        ...
                        <input type="text" id="name" name="name" required value={formData.name} onChange={handleChange} className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all" placeholder="John Doe" />
                      </div>
                      <div>
                        <label htmlFor="email" className="block text-sm font-semibold text-slate-900 mb-3">
                          Email Address *
                        </label>
                        <input type="email" id="email" name="email" required value={formData.email} onChange={handleChange} className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all" placeholder="john@example.com" />
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <select id="subject" name="subject" required value={formData.subject} onChange={handleChange} className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-900 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all">
                    ...
                    <div>
                      ...
                      <textarea id="message" name="message" rows={6} required value={formData.message} onChange={handleChange} className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all resize-none" placeholder="How can we help you?" />
                    </div>
            ...
            <div className="bg-white border border-slate-200/80 rounded-3xl p-4 shadow-xl shadow-primary/5 h-full min-h-[500px] flex flex-col">
              ...
              <div className="flex-grow w-full rounded-2xl overflow-hidden border border-slate-100 relative bg-slate-100">
                <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3610.178510024345!2d55.2721877!3d25.1972018!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3e5f682829c85c07%3A0xa4ec9701a0808600!2sAl%20Diyar%20General%20Trading%20Co.%20LLC!5e0!3m2!1sen!2sae!4v1710000000000!5m2!1sen!2sae" width="100%" height="100%" style={{
                border: 0,
                minHeight: "400px"
              }} allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" title="Enhance Tech Location" className="absolute inset-0" />
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-20 page-section-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {offices.map((office, index) => <div key={office.city} className="group bg-white border border-slate-200/80 rounded-3xl p-8 hover:border-transparent hover:shadow-xl hover:shadow-primary/10 transition-all duration-500 hover:-translate-y-1">
